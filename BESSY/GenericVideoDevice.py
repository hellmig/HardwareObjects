...
class GenericVideoDevice(Device):
    ...
    def get_new_image(self):
        ...
        if raw_buffer:
            ...
            # 2021-04-06-bessy-mh: no image conversion needed for prosilica in RGB24 format, to be extended for other formats
            # elif self.cam_type == "prosilica":
            elif self.cam_type in ("prosilica", "bzoom"):
                qimage = QtGui.QImage(raw_buffer, width, height,
                                      width * 3,
                                      QtGui.QImage.Format_RGB888)
            else:
