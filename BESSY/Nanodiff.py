...
class Nanodiff(Equipment):
    ...
    def init(self):
        self.centringMethods={Nanodiff.MANUAL3CLICK_MODE: self.start3ClickCentring,\
            Nanodiff.C3D_MODE: self.startAutoCentring }
        ...
        # 2016-02-26-bessy-mx-mh: old implementation in MiniDiff.py: why is the phiy Motor an object and not a device ???: self.phiyMotor = self.getObjectByRole("phiy")
        ...
        if self.phiMotor is not None:
            ...
            logging.getLogger("HWR").error('Nanodiff: phi motor is not defined in minidiff equipment %s', str(self.name()))
        if self.phizMotor is not None:
            ...
            logging.getLogger("HWR").error('Nanodiff: phiz motor is not defined in minidiff equipment %s', str(self.name()))
        if self.phiyMotor is not None:
            ...
            logging.getLogger("HWR").error('Nanodiff: phiy motor is not defined in minidiff equipment %s', str(self.name()))
        if self.zoomMotor is not None:
            ...
            logging.getLogger("HWR").error('Nanodiff: zoom motor is not defined in minidiff equipment %s', str(self.name()))
        if self.sampleXMotor is not None:
            ...
            logging.getLogger("HWR").error('Nanodiff: sampx motor is not defined in minidiff equipment %s', str(self.name()))
        ...
        if self.camera is None:
            logging.getLogger("HWR").error('Nanodiff: camera is not defined in minidiff equipment %s', str(self.name()))
        else:
            self.imgWidth, self.imgHeight = self.camera.getWidth(), self.camera.getHeight()
        if self.sampleChanger is None:
            logging.getLogger("HWR").warning('Nanodiff: sample changer is not defined in minidiff equipment %s', str(self.name()))
        else:
            try:
                self.connect(self.sampleChanger, 'sampleIsLoaded', self.sampleChangerSampleIsLoaded)
            except:
                logging.getLogger("HWR").exception('Nanodiff: could not connect to sample changer smart magnet')
        if self.lightWago is not None:
            self.connect(self.lightWago, 'wagoStateChanged', self.wagoLightStateChanged)
        else:
            logging.getLogger("HWR").warning('Nanodiff: wago light is not defined in minidiff equipment %s', str(self.name()))
    ...
    def zoomMotorPredefinedPositionChanged(self, positionName=None, offset=None):
        #print "zoomMotorPredefinedPositionChanged", positionName, offset
        ...
        self.emit('pixelsPerMmChanged', ((self.pixelsPerMmY, self.pixelsPerMmZ), ))
    ...
    def startCentringMethod(self,method,sample_info=None):
        ...
        if self.currentCentringMethod is not None:
            logging.getLogger("HWR").error("Nanodiff: already in centring method %s" % self.currentCentringMethod)
            return
