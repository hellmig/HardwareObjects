...
class XfeSpectrum(Equipment):
    def init(self):
        print "************************************* XfeSpectrum HO (BESSY version 2014) ********************************"
        self.scanning = None
        self.moving = None
        ...
        if True:
            ...
            self.sessionHO=self.getObjectByRole("session")
            if self.sessionHO is None:
                logging.getLogger().warning('XRFSpectrum: you should specify the session hardware object')
    ...
    def writeXfeSpectrumToDisk(self,directory,prefix,session_id=None,blsample_id=None):
...
# 2014-01-28-bessy-mh: begin - do not save to archive but to normal data-collection directory
...
# 2014-01-28-bessy-mh: disable copying of the png file because the target path does not
#                      exist on our system
