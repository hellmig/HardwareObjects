...
class DataCollectionQueueEntry(BaseQueueEntry):
    ...
    def collect_dc(self, dc, list_item):
        ...
        if self.collect_hwobj:
		    ...
		    # DJ: The store_centred_position method seems to be deprecated, so I'm commenting it out
		    # dc.lims_end_pos_id = self.lims_client_hwobj.store_centred_position(end_cpos)
		...
		# DJ: The store_centred_position appears to be deprecated, commenting it out
...
def mount_sample(beamline_setup_hwobj,
    ...
    if hasattr(sample_mount_device, '__TYPE__'):
        if sample_mount_device.__TYPE__ in ['Marvin','CATS','GROB_SC','ISARA2']:
            element = '%d:%02d' % loc
    ...
    if not sample_mount_device.hasLoadedSample():
        ...
        log.warning("Manual centring used, waiting for user to center sample")
