...
class Qt4_LimaVideo(Device):
    ...
    def init(self):
        ...
        if self.image_polling is None:
            self.video.setMode(Core.RGB24)
            self.video.startLive()
            self.video.setExposure(0.1)
            print "******************* Acq time", self.video.getExposure()
            self.change_owner()
    ...
    def get_new_image(self):
        ...
        if image.frameNumber() > -1:
            ...
            qimage = QtGui.QImage(raw_buffer, imgwidth, imgheight, bytesperline, QtGui.QImage.Format_RGB888)
