...
from utils.conversion import string_types
import HardwareRepository as HWR
...
class ISPyBClient(HardwareObject):
    ...
    def init(self):
        ...
        #self.pyispyb = self.getObjectByRole("pyispyb")
        #self.icat_client = self.getObjectByRole("icat_client")
    ...
    @property
    def loginType(self):
        return self.getProperty("loginType") or "proposal"
	...
	elif self.authServerType == "skip":
	   # David James: Implemented a 'skip' authentication type. If the user is using MXCuBE,
	   # then they have already authenticated into the OS and don't need to authenticate again
	   ok = True
        else:
    ...
    @trace
    def get_samples_as_objects(self, proposal_id, session_id):
	samples_as_dicts = self.get_samples(proposal_id, session_id)
	samples_as_objects = [SampleObject(**x) for x in samples_as_dicts]

	return samples_as_objects
    ...
    def store_data_collection_group(self, mx_collection):
        ...
        if self._collection:
            group_id = None
            if "ispyb_group_data_collections" in mx_collection:
