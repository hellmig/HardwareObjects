...
class Qt4_GraphicsManager(HardwareObject):
    ...
    def init(self):
        ...
        if self.diffractometer_hwobj:
            ...
            GraphicsItemGrid.set_grid_direction(self.diffractometer_hwobj.\
                 get_grid_direction())
...
class GraphicsItemPoint(GraphicsItem):
    ...
    def get_full_name(self):
        ...
        if self.__centred_position.kappa:
            kappa_str = "kappa: %0.2f" % self.__centred_position.kappa
        else:
            kappa_str = "kappa: n/a"
        if self.__centred_position.kappa_phi:
            phi_str = "phi: %0.2f" % self.__centred_position.kappa_phi
        else:
            phi_str = "phi: n/a"
        full_name = "Point %d (%s %s)" % (self.index, phi_str, kappa_str)
        return full_name
...
class GraphicsItemScale(GraphicsItem):
    ...
    def set_pixels_per_mm(self, pixels_per_mm):
        # print "***********", pixels_per_mm, all(pixels_per_mm)
        if all(pixels_per_mm):
            self.pixels_per_mm = pixels_per_mm
