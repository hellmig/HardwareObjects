...
class BESSYAbstractXRFSpectrum(object):
    """
    Descript. 
    """
    ...
    def start_spectrum(self, ct, spectrum_directory, archive_directory, prefix,
        ...
        if not os.path.isdir(spectrum_directory):
            logging.getLogger().debug("XRFSpectrum: creating directory %s" % spectrum_directory)
            try:
        ...
        if os.path.exists(spectrum_file_template + ".dat"):
            i = 1
            while os.path.exists(spectrum_file_template + "%d.dat" %i):
                  i = i + 1
            spectrum_file_template += "_%d" % i
            prefix += "_%d" % i

        spectrum_file_dat_filename = os.path.extsep.join((spectrum_file_template, "dat"))
        spectrum_file_png_filename = os.path.extsep.join((spectrum_file_template, "png"))
        spectrum_file_html_filename = os.path.extsep.join((spectrum_file_template, "html"))
        ...
        self.spectrum_info["jpegScanFileFullPath"] = spectrum_file_png_filename
        self.spectrum_info["exposureTime"] = ct
        self.spectrum_info["annotatedPymcaXfeSpectrum"] = spectrum_file_html_filename
        self.spectrum_info["htmldir"] = spectrum_directory
