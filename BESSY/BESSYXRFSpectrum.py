...
class BESSYXRFSpectrum(BESSYAbstractXRFSpectrum, HardwareObject):
    """
    Descript. 
    """
    ...
    def init(self):
        ...
        if self.transmission_hwobj is None:
            logging.getLogger("HWR").warning("BESSYXRFSpectrum: Transmission hwobj not defined")

        self.db_connection_hwobj = self.getObjectByRole("dbserver")
        if self.db_connection_hwobj is None:
            logging.getLogger().warning("BESSYXRFSpectrum: DB hwobj not defined")

        self.beam_info_hwobj = self.getObjectByRole("beam_info")
        if self.beam_info_hwobj is None:
            logging.getLogger("HWR").warning("BESSYXRFSpectrum: Beam info hwobj not defined")
    ...
    def execute_spectrum_command(self, count_sec, filename, adjust_transmission=True):
        try:
            ...
            logging.getLogger().exception('BESSYXRFSpectrum: problem in starting spectrum')
